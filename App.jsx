      ...
      <div className="flex-1 overflow-y-auto p-8 custom-scrollbar">
        {viewMode === 'edit' ? (
          <div className="space-y-6">
            <input value={localNote.title || ''} onChange={(e) => setLocalNote({...localNote, title: e.target.value})} placeholder="Title..." className="w-full bg-transparent text-3xl font-black text-white outline-none placeholder:text-slate-800" />
            <textarea value={localNote.content || ''} onChange={(e) => setLocalNote({...localNote, content: e.target.value})} placeholder="Start writing notes..." className="w-full h-[500px] bg-transparent text-slate-300 text-sm outline-none resize-none" />
          </div>
        ) : (
    ...
    <div className="min-h-screen bg-[#0a0f18] text-slate-200 pb-24 lg:pb-0 font-sans selection:bg-blue-500/30">
      ...
      <main className="max-w-7xl mx-auto px-6 py-10 lg:pl-32">
        <header className="mb-12 flex flex-col md:flex-row justify-between items-center gap-8 text-center md:text-left">
          <div><h1 className="text-4xl md:text-6xl font-black text-white italic uppercase tracking-tighter leading-none">COMMAND <span className="text-blue-500">PRO</span></h1><p className="text-slate-500 font-black uppercase text-[10px] tracking-[0.3em] mt-3">Readiness System 2026</p></div>
          <div className="bg-[#0f172a] border border-slate-800 rounded-3xl p-4 flex items-center gap-6 shadow-2xl">
            <div className="flex flex-col items-center min-w-[70px]"><span className="text-[8px] font-black text-slate-500 uppercase tracking-widest leading-none mb-1">Session</span><span className="text-2xl font-black text-white tabular-nums leading-none tracking-tighter">{Math.floor(timeLeft / 60)}:{String(timeLeft % 60).padStart(2, '0')}</span></div>
            <div className="flex gap-2">
          ...
          <div className="space-y-12 pb-20 animate-in fade-in duration-700">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
              <section className="space-y-6">
                <h2 className="text-xl font-black text-white uppercase italic tracking-tighter ml-2 leading-none">Academic Roadmap</h2>
                {EXAM_DATA.filter(e => e.id !== 'general').map(exam => (
                  <button key={exam.id} onClick={() => setSelectedExam(exam)} className={`w-full text-left bg-[#0f172a] border p-6 rounded-[36px] transition-all group flex items-center gap-6 ${exam.status === 'CRITICAL FOCUS' ? 'border-red-500/30' : 'border-slate-800'} hover:border-blue-500 shadow-xl`}>
                    ...
                    <div className="flex-1 min-w-0"><h4 className="text-white font-black italic text-xl truncate group-hover:text-blue-400 transition-colors leading-tight">{exam.subject}</h4><div className="flex items-center gap-3 mt-2"><div className="h-1 flex-1 bg-slate-800 rounded-full overflow-hidden"><div className="h-full bg-blue-500" style={{width: `${confidence[exam.id]||0}%`}}></div></div><span className="text-[9px] font-black text-slate-500 uppercase">{confidence[exam.id] || 0}% READY</span></div></div>
              ...
              <section className="bg-slate-900 border border-slate-800 rounded-[50px] p-10 flex flex-col items-center justify-center text-center space-y-7 relative overflow-hidden shadow-2xl group">
                 ...
                 <div><h3 className="text-3xl font-black text-white italic leading-tight">Cloud Sync Active</h3><p className="text-slate-500 text-xs mt-4 tracking-wide font-medium leading-relaxed">System status: Online. Database mirrored across 2026 ecosystem.</p></div>
          ...
          <div className="space-y-8 animate-in slide-in-from-right pb-20 max-w-4xl mx-auto">
            <div className="bg-[#0f172a] border border-slate-800 p-8 rounded-[40px] space-y-10 shadow-2xl">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div className="space-y-3"><label className="text-[10px] font-black text-slate-500 uppercase px-3 tracking-widest">Subject Selection</label><select value={selectedSubjectId} onChange={(e) => setSelectedSubjectId(e.target.value)} className="w-full bg-slate-900 border border-slate-800 text-white rounded-2xl p-5 font-black outline-none focus:ring-4 focus:ring-blue-500/10 shadow-xl cursor-pointer">{EXAM_DATA.filter(e => e.id !== 'general').map(e => <option key={e.id} value={e.id}>{e.subject}</option>)}</select></div>
              ...
              <div className="pt-10 border-t border-slate-800 space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  ...
                  <button onClick={() => { if(newResTitle && newResUrl) { const up = [{id: Date.now(), subjectId: selectedSubjectId, title: newResTitle, url: newResUrl}, ...resources]; setResources(up); sync('resources', up); setNewResTitle(''); setNewResUrl(''); } }} className="bg-green-600 text-white font-black p-4 rounded-2xl text-xs hover:bg-green-500 transition-all shadow-lg flex items-center justify-center gap-2 uppercase tracking-widest leading-none"><LinkIcon size={14}/> Link Asset</button>
                </div>
              </div>
                  ...
                  return (
                    <div key={subj.id} className="p-7 bg-slate-900/40 border border-slate-800 rounded-[40px] space-y-5 shadow-xl">
                      ...
                      <div className="space-y-3">
                        {sTasks.map(t => (
                          <div key={t.id} className="flex items-center gap-4 bg-slate-800/30 p-5 rounded-3xl border border-slate-800 group transition-all hover:border-blue-500/30 shadow-sm">
              ...
              <div className="flex flex-col md:flex-row gap-8 h-full">
                ...
                <div className="flex-1 space-y-10 overflow-y-auto pr-2 custom-scrollbar pb-32">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    {notes.length === 0 && <div className="col-span-full py-40 flex flex-col items-center justify-center opacity-10"><FileText size={80} className="mb-6"/><p className="text-xs font-black uppercase tracking-[0.5em]">Repository Empty</p></div>}
                  </div>
                </div>
              </div>
            )}
            ...
            <div className="flex flex-col lg:flex-row gap-10">
              <div className="flex-1 bg-[#0f172a] border border-slate-800 rounded-[50px] p-10 shadow-2xl">
                <div className="flex flex-col md:flex-row justify-between items-center mb-12 px-4 gap-6">
                  <h2 className="text-3xl font-black text-white italic uppercase tracking-tighter leading-none">Planner <span className="text-blue-500">2026</span></h2>
                  <div className="flex gap-6 items-center">
                    <div className="flex items-center gap-2 text-[10px] font-black text-slate-600 tracking-widest uppercase leading-none"><div className="w-2.5 h-2.5 rounded-full bg-red-500" /> Critical</div>
                    <div className="flex items-center gap-2 text-[10px] font-black text-blue-400 tracking-widest uppercase leading-none"><div className="w-2.5 h-2.5 rounded-full bg-blue-600 shadow-md shadow-blue-500/20" /> Selected</div>
                  </div>
                </div>
                  ...
                  {Array.from({length: 14}, (_, i) => 18 + i).map(day => {
                    ...
                    return (
                      <button key={`day-${day}`} onClick={() => setSelectedDate(day)} className={`relative aspect-square rounded-[38%] flex flex-col items-center justify-center transition-all border-2 ${isSelected ? 'bg-blue-600 border-blue-400 shadow-2xl scale-105 z-10' : 'bg-[#0a0f18] border-transparent hover:border-slate-800'}`}>
                        ...
                        {exams.some(e => e.status === 'CRITICAL FOCUS') && !isSelected && <div className="absolute top-2 right-2 w-2 h-2 bg-red-500 rounded-full animate-pulse shadow-lg shadow-red-500/40" />}
                      </button>
                    );
                  })}
                </div>
              ...
              <div className="w-full lg:w-[400px] bg-[#0f172a] border border-slate-800 rounded-[50px] p-10 h-fit shadow-2xl space-y-10">
                <div className="flex items-center gap-7"><div className="w-24 h-24 bg-blue-600 rounded-[35%] flex flex-col items-center justify-center shadow-2xl shadow-blue-600/30 shrink-0"><span className="text-[10px] font-black text-white/50 uppercase leading-none mb-1">MAY</span><span className="text-5xl font-black text-white leading-none tracking-tighter tabular-nums">{selectedDate}</span></div><h3 className="text-2xl font-black text-white tracking-tighter italic leading-none uppercase">Agenda<br/><span className="text-blue-500 text-sm tracking-[0.2em] font-black uppercase not-italic">Briefing</span></h3></div>
                <div className="space-y-6 max-h-[500px] overflow-y-auto pr-2 custom-scrollbar">
                  {EXAM_DATA.filter(e => e.date?.includes(`-05-${selectedDate}`)).map(exam => (
                    <div key={exam.id} className={`p-7 rounded-[40px] border transition-all ${exam.status === 'CRITICAL FOCUS' ? 'bg-red-500/5 border-red-500/20 shadow-lg' : 'bg-[#0a0f18] border-slate-800'} group hover:border-blue-500`}>
                      <div className="flex justify-between items-start mb-6"><span className={`px-5 py-1.5 rounded-full text-[9px] font-black uppercase ${exam.color} text-white shadow-xl shadow-black/20`}>{exam.status}</span><Clock size={16} className="text-slate-700 group-hover:text-blue-400 transition-colors"/></div>
                      <h4 className="text-white font-black text-2xl italic mb-2 tracking-tight leading-tight">{exam.subject}</h4>
                      <p className="text-slate-500 text-[10px] font-black uppercase tracking-widest mb-6 leading-none">{exam.time}</p>
                      <button onClick={() => setSelectedExam(exam)} className="w-full bg-slate-800 border border-slate-700 py-4 rounded-2xl text-[9px] font-black uppercase tracking-widest text-slate-300 hover:text-white hover:bg-blue-600 hover:border-blue-500 transition-all shadow-md active:scale-95">Subject Intel</button>
                    </div>
                  ))}
                  {EXAM_DATA.filter(e => e.date?.includes(`-05-${selectedDate}`)).length === 0 && <div className="text-center opacity-10 py-24 flex flex-col items-center"><Coffee size={50} className="mb-4"/><p className="text-[10px] font-black uppercase tracking-[0.4em] leading-none">Self-Study Window</p></div>}
                </div>
              </div>
            </div>
          </div>
